...
>(({ onLoginSuccess, onSignUpPress, onDismiss }, ref) => {
  ...
  const { mutateAsync: loginUser, isPending: isLoggingIn } = useLogin({
    onSuccess: async (data) => {
      ...
      if (guestSessionId) {
        console.log(
          "[LoginBottomSheet] Guest session detected, cart will be auto-merged"
        );
        clearGuestSession();
      }
      ...
      console.log(
        "[DEBUG] ========== CART RESTORATION (BottomSheet) =========="
      );
      console.log("[DEBUG] shopifyCartId from login response:", shopifyCartId);

      if (shopifyCartId) {
        console.log(
          "[DEBUG] Found shopifyCartId, attempting to restore cart..."
        );
        try {
          ...
          if (cartResponse?.data?.items && cartResponse.data.items.length > 0) {
            ...
            console.log(
              "[DEBUG] ✅ Cart restored successfully:",
