...
Known gap: Runtime branding is implemented (colors + logo, PRs #115/#116), but two coverage gaps remain: the hardcoded `t("common.companyName")` string isn't wired to dashboard branding, and a handful of `getTokenValue()`/`AppImage` tint call sites don't reactively pick up a runtime color change. See `docs/MOBILE_BRANDING_SPLIT_MATRIX.md` for the full surface-by-surface accounting. No live dashboard-edit → backend → mobile-app integration run has been recorded end-to-end; each stage's evidence today is component-level (backend contract, mobile hooks, dashboard editor tested independently), not a single connected trace.
