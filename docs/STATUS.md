...
Last updated: 2026-07-09 (GitHub issue #80 re-ran the mobile/backend smoke suites against the currently configured Railway backend; the endpoint returned platform-level `404 Application not found`, so `/cart/*` plus Shopify-hosted `/checkout/handoff` remain blocked on a reachable backend sandbox with store-scoped Shopify Storefront credentials).
