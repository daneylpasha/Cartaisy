...
Current state: As of 2026-07-08 (GitHub issue #73), `eas.json` includes a non-secret `sample-merchant-development` profile for the fictional Acme sample in `docs/examples/sample-merchant.env`. Local Expo config verification resolves the Acme app identity, but no remote EAS artifact exists yet: the sample EAS project ID is a placeholder, and the attempted Android development build was blocked before uploading the build archive to Expo's external EAS service.
...
Known gap: Dynamic app config output was verified with a sample merchant through `npx expo config` and `npx expo prebuild` (2026-07-02, findings in `docs/MOBILE_BRANDED_BUILD_CHECKLIST.md`) and re-verified through `npx expo config --type public --json` on 2026-07-08. The checked-in-native-projects bypass was resolved on 2026-07-03 (`.easignore`, GitHub issue #60), and a non-secret sample EAS profile now exists, but no EAS or signed merchant build artifact has been produced. Merchant builds still need an accessible EAS project/owner, matching Firebase files, remote credentials/signing setup, and an approved environment for uploading the EAS build archive.
