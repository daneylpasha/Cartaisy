...
Known gap: Dynamic app config output was verified with a sample merchant through `npx expo config` and `npx expo prebuild` (2026-07-02, findings in `docs/MOBILE_BRANDED_BUILD_CHECKLIST.md`) and re-verified through `npx expo config --type public --json` on 2026-07-08, 2026-07-09, and 2026-07-13. The checked-in-native-projects bypass was resolved on 2026-07-03 (`.easignore`, GitHub issue #60), and the sample profile now points at a real internal sample EAS project, but no EAS or signed merchant build artifact has been produced. The 2026-07-13 Android EAS development build attempt was blocked by uninitialized Android remote credentials/keystore in `--non-interactive` mode. Merchant builds still need initialized remote credentials/signing setup, matching Firebase files supplied through secure EAS file environment variables, a clean approved environment for uploading the EAS build archive, and installed-artifact validation.
