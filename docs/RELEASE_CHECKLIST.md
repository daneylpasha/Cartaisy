...
Known gap: Dynamic app config output was verified with a sample merchant through `npx expo config` and `npx expo prebuild` (2026-07-02, findings in `docs/MOBILE_BRANDED_BUILD_CHECKLIST.md`) and re-verified through `npx expo config --type public --json` on 2026-07-08 and 2026-07-09. The checked-in-native-projects bypass was resolved on 2026-07-03 (`.easignore`, GitHub issue #60), and a non-secret sample EAS profile now exists, but no EAS or signed merchant build artifact has been produced. The 2026-07-09 Android EAS development build attempt was blocked by the placeholder sample project ID before archive upload. Merchant builds still need an accessible EAS project/owner, matching Firebase files, remote credentials/signing setup, and a clean approved environment for uploading the EAS build archive.
