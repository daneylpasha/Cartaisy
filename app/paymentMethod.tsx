...
import { useListPaymentMethods, useRemovePaymentMethod, useSetDefaultPaymentMethod } from "@/api/generated/payment-methods/payment-methods";
...
const PaymentMethod = () => {
  ...
  const { data: paymentMethodsResponse, isLoading, refetch } = useListPaymentMethods();
                  ...
                  <YStack position="relative" borderRadius="$xl" height={200}>
                    ...
                    <YStack position="absolute" width={"80%"} bottom={20} left={40}>
                      ...
                      >
                        ...
                        <LabelMD color={"$secondary"} marginBottom={2}>
                          {defaultCard.card?.exp_month?.toString().padStart(2, '0')}/{defaultCard.card?.exp_year?.toString().slice(-2)}
                        </LabelMD>
                      </XStack>
                      <XStack justifyContent="space-between" alignItems="center">
                        <TextLGBold>•••• •••• •••• {defaultCard.card?.last4}</TextLGBold>
                        <YStack
                          width={48}
                          height={33}
                          borderWidth={0.7}
                      ...
                      <XStack gap="$xs" alignItems="center">
                        ...
                        {card.isDefault && (
                          <YStack
                            ...
                            borderRadius="$md"
                          >
                            <TextSMSemiBold fontSize={10} color="$white">DEFAULT</TextSMSemiBold>
                          </YStack>
                        )}
                      </XStack>
                      <Spacer size="$xs" />
                      ...
                      {!card.isDefault && (
                        <OpTouch onPress={() => handleSetDefault(card.id)}>
                          <YStack
                            ...
                            borderRadius="$md"
                          >
                            <TextSMSemiBold color="$primary">Set Default</TextSMSemiBold>
                          </YStack>
                        </OpTouch>
                      )}
