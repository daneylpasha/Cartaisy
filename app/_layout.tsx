...
> {
  ...
  try {
    ...
    if (!Device.isDevice) {
      ...
      console.log(
        "🔔 [PUSH] Push notifications require a physical device, not simulator/emulator"
      );
      return undefined;
    }
    ...
    try {
      ...
      console.log(
        "🔔 [PUSH] Step 3: Has getToken:",
        typeof messagingInstance.getToken === "function"
      );
    } catch (msgError: any) {
    ...
    if (Platform.OS === "ios") {
      ...
      try {
        ...
        console.log(
          "🔔 [PUSH] Step 4: iOS permission error:",
          iosError.message
        );
      }
    } else {
    ...
    try {
      ...
      console.log(
        "🔔 [PUSH] Step 5: Token preview:",
        token?.substring(0, 40) + "..."
      );
      console.log(
        "🔔 [PUSH] Step 5: Token type:",
      ...
      if (tokenError.message?.includes("MISSING_INSTANCEID_SERVICE")) {
        console.log(
          "🔔 [PUSH] Step 5: Google Play Services not available on this device/emulator"
        );
      }
      if (tokenError.message?.includes("SERVICE_NOT_AVAILABLE")) {
        console.log(
          "🔔 [PUSH] Step 5: Firebase service not available - check google-services.json"
        );
      }
      if (tokenError.message?.includes("TOO_MANY_REGISTRATIONS")) {
        console.log(
          "🔔 [PUSH] Step 5: Too many registrations - uninstall and reinstall app"
        );
      }
...
export default function RootLayout() {
  ...
  useEffect(() => {
    console.log(
      "🔔 [PUSH] useEffect TRIGGERED - about to call registerForPushNotificationsAsync"
    );
    ...
    registerForPushNotificationsAsync()
      .then((token) => {
        ...
        if (token) {
          ...
          console.log(
            "⚠️ [PUSH] Token not generated - check logs above for failure point"
          );
        }
      })
      .catch((error) => {
    ...
    notificationTracker.processQueue().catch((error) => {
      console.error(
        "🔔 [PUSH] Failed to process notification tracking queue:",
        error
      );
    });
    ...
    const subscription = Notifications.addNotificationReceivedListener(
      (notification) => {
        ...
        const notificationId = notificationData?.notificationId as
          | string
          | undefined;
        const deviceId = useAuthStore.getState().deviceId || undefined;
        ...
        if (notificationId && isFromBackground) {
          console.log(
            "🔔 [PUSH] Tracking delivery (background) for:",
            notificationId
          );
          notificationTracker.trackDelivery(notificationId, deviceId, {
    ...
    const responseSub = Notifications.addNotificationResponseReceivedListener(
      async (response) => {
        console.log(
          "🔔 [PUSH] Step 8: Notification tapped (response listener)!"
        );
        console.log(
          "🔔 [PUSH] Step 8a: Title:",
        ...
        if (notificationId) {
          console.log(
            "🔔 [PUSH] Tracking open for notificationId:",
            notificationId
          );
          ...
          if (
            ...
            await notificationTracker.trackClick(
              notificationId,
              actionId,
              deviceId,
              {
          ...
          console.warn(
            "🔔 [PUSH] No notificationId in payload, skipping open tracking"
          );
        }
        ...
        if (notificationData) {
          console.log(
            "🔔 [PUSH] Checking for deep link in notification data..."
          );
          ...
          if (handled) {
            ...
            console.log(
              "🔔 [PUSH] No deep link found, staying on current screen"
            );
          }
        }
      }
  ...
  useEffect(() => {
    const sendTokenIfReady = async () => {
      ...
      console.log(
        "🔔 [PUSH] - Token already sent this session:",
        tokenSentRef.current
      );
      ...
      if (_hasHydrated && expoPushToken && authToken && !tokenSentRef.current) {
        ...
        console.log(
          "🔔 [PUSH] Token ready but user not logged in - will send after login"
        );
      }
    };
  ...
  useEffect(() => {
    const handleInitialNotification = async () => {
      ...
      try {
        console.log(
          "🔔 [PUSH] Checking for initial notification (cold start)..."
        );
        ...
        if (response) {
          ...
          if (notificationId) {
            console.log(
              "🔔 [PUSH] Cold start: Tracking open for:",
              notificationId
            );
            await notificationTracker.trackOpen(notificationId, deviceId, {
              title: response.notification.request.content.title,
          ...
          if (notificationData) {
            console.log(
              "🔔 [PUSH] Cold start: Processing deep link with delay..."
            );
            const handled = handleNotificationDeepLink(
              notificationData,
              router,
              {
            ...
            if (handled) {
              console.log(
                "🔔 [PUSH] Cold start: Deep link navigation scheduled"
              );
              // Mark that deep link was handled - prevents splash from overriding
              setDeepLinkHandled(true);
            } else {
        ...
        console.error(
          "🔔 [PUSH] Cold start: Error checking initial notification:",
          error
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (pendingInitialNotification) {
        console.log(
          "🔔 [PUSH] Killed state: Processing pending notification now..."
        );
        console.log(
          "🔔 [PUSH] Killed state: Data:",
          JSON.stringify(pendingInitialNotification.data, null, 2)
        );

        const notificationData = pendingInitialNotification.data as
          | NotificationDeepLinkPayload
          | undefined;
        const notificationId = notificationData?.notificationId as
          | string
        ...
        if (notificationId) {
          console.log(
            "🔔 [PUSH] Killed state: Tracking open for:",
            notificationId
          );
        ...
        if (notificationData) {
          ...
          if (handled) {
            console.log(
              "🔔 [PUSH] Killed state: Deep link navigation scheduled"
            );
            setDeepLinkHandled(true);
          }
        }
