...
- If the task refers to a GitHub issue number, the PR description must include a GitHub closing keyword for that exact issue number, such as `Closes #12`, so GitHub automatically closes the issue when the PR is merged. Do not use a placeholder or a different issue number.
